    ...
    for i in range(0, len(list)):
        ...
        if extension == 'json':
            if os.path.isfile(path):
                ...
                #     os.mkdir(out_dir)
                PIL.Image.fromarray(img).save(osp.join("nnUNet_raw", "source", '{}_0000.png'.format(filename)))
                PIL.Image.fromarray(lbl).save(osp.join("nnUNet_raw", "mask", '{}.png'.format(filename)))
